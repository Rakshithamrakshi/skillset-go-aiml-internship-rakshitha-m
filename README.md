# Skill Set Go EduTech
## AI & Machine Learning Internship

### Intern Details
- **Name:** Rakshitha M
- **Offer Letter ID:** SSG/AIML/B1/0014
- **Domain:** AI & Machine Learning
- **Batch:** B1
- **Internship Duration:** September - October 2026
- **GitHub Profile:** https://github.com/Rakshithamrakshi
- **LinkedIn Profile:** https://www.linkedin.com/in/rakshitha-m-758332389
---

## Internship Progress

| Week | Task | Status | Submission |
|------|------|--------|-----------|
| Week 1 | Python Fundamentals | Completed | [View](Week-1/Task-1.1-Python-Fundamentals) |
| Week 1 | Data Cleaning & EDA | Completed | [View](Week-1/Task-1.2-Data-Cleaning-EDA) |
| Week 1 | Math for ML | Completed | [View](Week-1/Task-1.3-Math-for-ML) |
| Week 1 | Scikit-Learn Models | Completed | [View](Week-1/Task-1.4-Scikit-Learn-Models) |
| Week 2 | PyTorch Neural Network | Pending | - |
| Week 2 | CNN Image Classifier | Pending | - |
| Week 2 | Hyperparameter Experiments | Pending | - |
| Week 2 | Technical Report | Pending | - |
| Week 3 | TensorFlow/Keras Model | Pending | - |
| Week 3 | CV/Text Classification | Pending | - |
| Week 3 | Prompt Engineering | Pending | - |
| Week 3 | AI-Assisted Coding | Pending | - |
| Week 4 | RAG Application | Pending | - |
| Week 4 | LangGraph Workflow | Pending | - |
| Week 4 | LangSmith Evaluation | Pending | - |
| Week 4 | AI Agent Capstone | Pending | - |

---

## Weekly Progress

### Week 1 - Python, Mathematics & ML Foundations
Completed all 4 tasks: Python fundamentals (6 topics with tests), a full 
data cleaning and EDA workflow on the Titanic dataset, worked math solutions 
connecting statistics/probability/linear algebra/calculus to ML concepts, 
and two evaluated Scikit-learn models (regression + classification) with 
error analysis.

### Week 2 - Deep Learning with PyTorch
### Week 3 - TensorFlow, GenAI & Prompt Engineering
### Week 4 - LangChain, LangGraph & AI Agents

---

## Final Capstone
AI Agent System
GitHub: 
Live Demo: 
Report: 
Presentation: 

---

## Skill Set Go EduTech
Learn. Build. Prove.